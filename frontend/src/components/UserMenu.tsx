...
const USE_MOCK_AUTH = ((import.meta as any)?.env?.VITE_USE_MOCK_AUTH === 'true') || !((import.meta as any)?.env?.VITE_API_BASE_URL);
                    ...
                    {!user ? (
                        <form className="space-y-3" onSubmit={handleLoginSubmit} noValidate>
                            <div className="flex flex-col gap-1">
                                <label htmlFor="um-username" className="text-sm">User Name</label>
                                <input
                                    id="um-username"
                                    name="username"
                            ...
                            <div className="flex flex-col gap-1">
                                <label htmlFor="um-password" className="text-sm">Password</label>
                                <input
                                    id="um-password"
                                    name="password"
                                    type="password"
                            ...
                            {USE_MOCK_AUTH && (
                                <div className="text-xs text-gray-500">Demo creds: <code>demo</code> / <code>demo12345</code></div>
                            )}
                            {error && (
                                <div className="text-red-700 text-sm" role="alert" aria-live="assertive">{error}</div>
                            )}
                            <div className="flex justify-between items-center">
                                <button className="px-3 py-1 border rounded disabled:opacity-50 hover:bg-emerald-200 border-green-900"  type="submit" disabled={submitting}>
